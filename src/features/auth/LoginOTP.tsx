...
// Login sem senha e sem e-mail: só o número. Recebe um código, digita, entrou.
// Duas etapas em tela cheia, uma coisa por vez.

function paraE164(bruto: string): string {
  const digitos = bruto.replace(/\D/g, '')
  // assume Brasil se a pessoa não colocou o país
  if (digitos.startsWith('55')) return '+' + digitos
  return '+55' + digitos
}

export function LoginOTP({ compacto = false }: { compacto?: boolean }) {
  const { enviarCodigo, confirmarCodigo, telefoneEmVerificacao } = useAuth()
  ...
  const etapaCodigo = Boolean(telefoneEmVerificacao)

  async function pedirCodigo() {
    setErro(null)
    if (nome.trim().length < 2) return setErro('Escreva seu nome.')
    if (telefone.replace(/\D/g, '').length < 10)
      return setErro('Coloque seu número com DDD.')
    setOcupado(true)
    try {
      await enviarCodigo(paraE164(telefone), nome.trim())
    } catch (e) {
      const msg = e instanceof Error ? e.message : ''
      // mostra o motivo real (ex.: provedor não configurado) além da dica amigável
      setErro(
        msg
  ...
  async function entrar() {
    ...
    try {
      ...
      setErro('Código errado. Confira e tente de novo.')
    } finally {
      setOcupado(false)
    }
  }
      ...
      {!etapaCodigo ? (
        <div className="space-y-4">
          <label className="block">
            <span className="mb-1 block font-bold">Como te chamam?</span>
            <input
              value={nome}
          ...
          <p className="text-center text-sm text-tinta-suave">
            A gente manda um código no seu celular pra confirmar que é você.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          <button
            onClick={() => useAuth.setState({ telefoneEmVerificacao: null })}
